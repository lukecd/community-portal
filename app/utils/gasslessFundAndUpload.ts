 ...
 */
const gasslessFundAndUpload = async (data: string, tags: Tag[]): Promise<string> => {
	console.log("gasslessFundAndUpload 1");
		...
		getSigner: () => {
			return {
				...
				_signTypedData: async (
					...
					console.log(
						"JSON.stringify({ signatureData: convertedMsg }: ",
						JSON.stringify({ signatureData: convertedMsg }),
					);
	...
	const tx = await bundlr.uploadWithReceipt(data, {
		tags,
	});
